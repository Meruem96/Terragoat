...
az group create --location $TF_VAR_region --name $TERRAGOAT_RESOURCE_GROUP >> .log/setupoutput.log && echo "OK"

# Create storage account
az storage account create --name $TERRAGOAT_STATE_STORAGE_ACCOUNT --resource-group $TERRAGOAT_RESOURCE_GROUP --location $TF_VAR_region --sku Standard_LRS --kind StorageV2 --https-only true --encryption-services blob >> .log/setupoutput.log && echo "Storage account ...OK"

# Get storage account key
echo -n "Storage account key ..."
ACCOUNT_KEY=$(az storage account keys list --resource-group $TERRAGOAT_RESOURCE_GROUP --account-name $TERRAGOAT_STATE_STORAGE_ACCOUNT --query [0].value -o tsv) >> .log/setupoutput.log && echo "OK"

# Create blob container
echo -n "Blob container ..."
az storage container create --name $TERRAGOAT_STATE_CONTAINER --account-name $TERRAGOAT_STATE_STORAGE_ACCOUNT --account-key $ACCOUNT_KEY >> .log/setupoutput.log && echo "OK"
...
terraform init -reconfigure -backend-config="resource_group_name=$TERRAGOAT_RESOURCE_GROUP" \
    ...
    -backend-config "key=$TF_VAR_environment.terraform.tfstate" >> .log/setupoutput.log && echo "OK"
